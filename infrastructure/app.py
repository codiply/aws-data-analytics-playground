...
config_directory: str = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "config"
)
pipeline_name: typing.Optional[str] = app.node.try_get_context("pipeline_name")
pipeline_config: PipelineConfig = PipelineConfig(
    config_directory=config_directory, pipeline_name=pipeline_name
)
...
CdkPipelineStack(
    ...
    pipeline_config,
    env=cdk.Environment(
        account=ci_cd_config["AccountId"], region=ci_cd_config["Region"]
    ),
)
