...
class BaseStack(cdk.Stack):
    ...
    def _define_s3_bucket(self):
        ...
        self.s3_bucket = s3.Bucket(
            ...
            lifecycle_rules=[
                s3.LifecycleRule(
                    expiration=cdk.Duration.days(s3_bucket_config["ExpirationDays"])
                )
            ],
        )
    ...
    def _define_roles(self):
        self.data_brew_role: iam.Role = DataBrewRole(
            self, "databrew-role", self._config.for_sections([])
        ).role

        self.glue_role: iam.Role = GlueRole(
            self, "glue-role", self._config.for_sections([])
        ).role

        self.glue_notebook_role: iam.Role = GlueNotebookRole(
            self, "glue-notebook-role", self._config.for_sections([])
        )
