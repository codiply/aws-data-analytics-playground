...
class MainStage(cdk.Stage):
    def __init__(
        self, scope: cdk.Construct, id: str, config: EnvironmentConfig, **kwargs
    ):
        ...
        if config.stack_enabled("Common"):
            common_stack: CommonStack = CommonStack(
                ...
                config.for_sections(["EcsCluster", "Vpc"]),
            )
        ...
        if config.stack_enabled("Testing"):
            TestingStack(
                ...
                vpc=common_stack.vpc,
                redshift_client_sg=data_warehouse_stack.redshift_client_sg
                if data_warehouse_stack
                else None,
                relational_database_client_sg=(
                    relational_database_stack.database_client_sg
                    if relational_database_stack
                    else None
                ),
            )
