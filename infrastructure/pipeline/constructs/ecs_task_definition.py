...
class EcsTaskDefinition(cdk.Construct):
    def __init__(
        ...
        self._policy_statements: typing.Sequence[
            iam.PolicyStatement
        ] = policy_statements
        ...
        task_definition.add_container(
            ...
            logging=ecs.LogDriver.aws_logs(
                stream_prefix=resource_prefix, log_retention=log_retention
            ),
        )
        ...
        if xray_enabled:
            task_definition.add_container(
                ...
                port_mappings=[
                    ecs.PortMapping(
                        container_port=xray_port, protocol=ecs.Protocol.TCP
                    ),
                    ecs.PortMapping(
                        container_port=xray_port, protocol=ecs.Protocol.UDP
                    ),
                ],
            )
